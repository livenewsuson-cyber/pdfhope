...
PDFHope is a local-first PDF utility website built with React, TypeScript, Vite, `pdf-lib`, and PDF.js. Current tools read and process selected files in the browser; the application does not include a PDF-upload endpoint.
...
If the Cloudflare local runtime cannot write its registry on a restricted workstation, use the plain Vite preview mode:
...
3. In the Cloudflare dashboard, attach the `pdfhope.com` custom domain to the deployed `pdfhope` Worker.
...
- There is no Worker route that accepts PDF file uploads.
- File bytes stay in browser memory during supported tool operations.

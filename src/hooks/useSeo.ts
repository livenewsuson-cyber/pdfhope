...
export function useSeo(title: string, description: string, path: string, index = true) {
  useEffect(() => {
    document.title = `${title} | PDFHope`
    ...
    setMeta('description',description);setMeta('robots',index?'index,follow':'noindex,follow');setMeta('og:title',title,true);setMeta('og:description',description,true);setMeta('og:type','website',true);setMeta('twitter:card','summary')
